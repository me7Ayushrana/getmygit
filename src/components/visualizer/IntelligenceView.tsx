            ...
            <div className="max-w-5xl mx-auto space-y-8">
                ...
                <div className="flex items-center justify-between mb-12">
                    ...
                    >
                        ...
                        <p className="text-gray-500 dark:text-zinc-500 text-sm mt-2 font-light tracking-wide uppercase">Deep architectural auditing & risk assessment</p>
                    </motion.div>
                    
                    <motion.div 
                        initial={{ opacity: 0, scale: 0.8 }}
                        animate={{ opacity: 1, scale: 1 }}
                        className="px-4 py-2 rounded-full border border-black/[0.06] dark:border-white/5 bg-black/[0.03] dark:bg-white/5 backdrop-blur-xl flex items-center gap-2"
                    >
                        <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        <span className="text-[10px] font-mono font-bold tracking-widest text-gray-500 dark:text-zinc-400">ENGINE_OPERATIONAL</span>
                    </motion.div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    >
                        <div className="glass-card relative p-8 rounded-3xl h-full transition-all duration-[250ms] ease-out flex items-center justify-between">
                            <div>
                                <div className="flex items-center gap-2 mb-3">
                                    <h3 className="text-xl font-display font-bold text-gray-900 dark:text-white/90">Health Score</h3>
                                    <Tooltip text="A composite metric analyzing code modularity, dependency coupling, and file complexity. A higher score indicates a more maintainable and scalable architecture." position="top">
                                        <Info size={14} className="text-gray-400 dark:text-zinc-500 hover:text-white transition-colors cursor-help" />
                                    </Tooltip>
                                </div>
                                <p className="text-gray-500 dark:text-zinc-500 text-sm leading-relaxed max-w-[200px] font-light">
                    ...
                    >
                        <div className="glass-card relative p-8 rounded-3xl h-full transition-all duration-[250ms] ease-out">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-xl font-display font-bold flex items-center gap-3 text-gray-900 dark:text-white/90">
                ...
                >
                    <div className="glass-card relative p-10 rounded-3xl transition-all duration-[250ms] ease-out">
                        <div className="flex items-center justify-between mb-10">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-2xl bg-rose-500/10 flex items-center justify-center border border-rose-500/20">
                                    <ShieldAlert size={24} className="text-rose-500" />
                                </div>
                                <h3 className="text-2xl font-display font-bold text-gray-900 dark:text-white">Critical Risk Mapping</h3>
                            ...
                            <span className="text-[10px] font-mono font-bold text-gray-400 dark:text-zinc-600 tracking-[0.3em] uppercase">Security Perimeter</span>
                        </div>
                        ...
                        <div className="space-y-4">
                            {calculatedRisks.length > 0 ? calculatedRisks.map((file, i) => (
                                <motion.div 
                                    ...
                                    className="group flex items-center justify-between p-6 rounded-2xl bg-black/[0.02] dark:bg-white/[0.02] border border-black/[0.06] dark:border-white/[0.04] hover:bg-black/[0.04] dark:hover:bg-white/[0.04] hover:border-black/[0.12] dark:hover:border-white/10 transition-all duration-300"
                                >
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                <p className="font-mono text-sm text-gray-800 dark:text-zinc-100 group-hover:text-gray-900 dark:group-hover:text-white transition-colors tracking-tight">{file.path}</p>
                                                <Tooltip text={`${file.reason}: This file modification requires mandatory senior review due to its architectural impact.`} position="top">
                                                    <Info size={12} className="text-gray-400 dark:text-zinc-500 hover:text-zinc-300 cursor-help" />
                                                </Tooltip>
                                            </div>
                                            <p className="text-[10px] text-gray-400 dark:text-zinc-500 mt-1 font-mono uppercase tracking-widest">{file.reason}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        ...
                                        <p className="text-[9px] text-gray-400 dark:text-zinc-600 font-mono mt-2 tracking-widest">{file.size?.toLocaleString()} BYTES</p>
                                    </div>
                                </motion.div>
                            )) : (
                                <div className="text-center py-20 bg-black/[0.01] dark:bg-white/[0.01] rounded-3xl border border-black/[0.06] dark:border-white/[0.02]">
                                    <ShieldCheck size={64} className="mx-auto text-emerald-500/30 mb-6" />
                                    <h4 className="text-lg font-display font-bold text-gray-500 dark:text-zinc-400">Security Perimeter Intact</h4>
                                    <p className="text-gray-500 dark:text-zinc-600 text-sm mt-2 max-w-xs mx-auto">No critical structural or configuration risks identified in this scan.</p>
                                </div>
                            )}
                        </div>
                    </div>
    ...
    return (
        <div className="space-y-2.5">
            <div className="flex justify-between items-end">
                <span className="text-gray-400 dark:text-zinc-400 text-xs font-mono font-bold tracking-widest uppercase">{label}</span>
                <span className="text-gray-900 dark:text-white text-xs font-bold">{value}</span>
            </div>
            <div className="h-1.5 w-full bg-black/[0.05] dark:bg-white/5 rounded-full overflow-hidden border border-black/[0.06] dark:border-white/[0.02]">
                <motion.div 
                    initial={{ width: 0 }}
                    animate={{ width: `${score}%` }}
